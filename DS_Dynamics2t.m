...
%outputs the dynamical variables as a function of time. The dynamical
...
load('../Codes_Networks/RG_k12_N1000')
addpath('../Codes_DS')
...
rvect = rand(N+L,1); % a random vector
...
for idx1 = 1:length(time) %time loop
...
if net_type == "RG"
    formatSpec = sprintf('DS_Dyn2t_m%d_E%d_lam%d_%d_RG_k%d_N%d',round(m),round(E),ismember(E,slam),isequal(round(abs(E),3),abs(m)),c,N);
elseif net_type == "SBM"
    formatSpec = sprintf('DS_Dyn2t_m%d_E%d_lam%d_%d_SBM_M%d_N%d',round(m),round(E),ismember(E,slam),isequal(round(abs(E),3),abs(m)),M,N);
end
