...
%outputs the dynamical variables as a function of sigma.
...
load('../Codes_Networks/SBM_M4_N100_6')
addpath('../Codes_DS')
...
R_alpha_fore = zeros(length(Sigma),1);
R_beta_fore = zeros(length(Sigma),1);
Psi_dot_fore = zeros(N+L,length(Sigma));
Psi_fore = zeros(N+L,length(Sigma));
...
for idx1 = 1:length(Sigma)
    ...
    beta = zeros(L,length(time));
    R_alpha = zeros(length(time),1);
    R_beta = zeros(length(time),1);
    ...
    for idx2 = 1:length(time) %time loop
        ...
        DPsi = Deqn*Psi;
        alpha(:,idx2) = DPsi(1:N);
        beta(:,idx2) = DPsi(N+1:N+L);

        if idx2 >= length(time)*ratio
            idx3 = idx3+1;
            R_alpha(idx3) = abs(sum(exp(1i*alpha(:,idx2))))/N;
            R_beta(idx3) = abs(sum(exp(1i*beta(:,idx2))))/L;
            Psi_dot_vect(:,idx3) = Psi_dot;
            Psi_vect(:,idx3) = Psi;
        end

    end
    avg_Psi_dot = mean(Psi_dot_vect(:,1:idx3),2);
    Psi_dot_fore(:,idx1) = avg_Psi_dot;
    avg_Psi = mean(Psi_vect(:,1:idx3),2);
    Psi_fore(:,idx1) = avg_Psi;
    R_alpha_fore(idx1) = mean(R_alpha(1:idx3));
    R_beta_fore(idx1) = mean(R_beta(1:idx3));
    
end

%%Set name of output file
if net_type == "RG"
    formatSpec = sprintf('DS_Dyn1s_m%d_E%d_lam%d_%d_RG_k%d_N%d',round(m),round(E),ismember(E,slam),isequal(round(abs(E),3),abs(m)),c,N);
elseif net_type == "SBM"
    formatSpec = sprintf('DS_Dyn1s_m%d_E%d_lam%d_%d_SBM_M%d_N%d',round(m),round(E),ismember(E,slam),isequal(round(abs(E),3),abs(m)),M,N);
end
